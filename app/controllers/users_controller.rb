class UsersController < ApplicationController
    ...
    def confirm
        ...
        if user.present? && user.confirmation_token_valid?
            ...
            #vérification si l'utitlisateur est propriétaire ou client
            if user.is_client == true
                redirect_to "#{ENV["CLIENT_URL"]}confirmed_client" # <-- redirect to the client domain name
            else
                redirect_to "#{ENV["CLIENT_URL"]}confirmed_prop" # <-- redirect to the client domain name
            end
        else
